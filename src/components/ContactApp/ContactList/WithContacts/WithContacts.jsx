...
const WithContacts = function () {
    ...
    const data = JSON.parse(localStorage.getItem(states.currentActiveAccount));
    ...
    const wrapperRightClickHandler = function ({
        ...
        if (target !== currentTarget) {
            const contactsDetails = JSON.parse(
                localStorage.getItem(states.currentActiveAccount)
            ).contactList[target.id];
    ...
    return (
        ...
        >
            ...
            >
                {data.contactList.map((items, index) => (
